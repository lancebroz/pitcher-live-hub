...
export async function getStatcast(pitcherId, startDate, endDate, gameType = "R") {
  try {
    const res = await fetch(
      `${API_BASE}/api/pitcher/${pitcherId}/statcast?start_date=${startDate}&end_date=${endDate}&game_type=${gameType}`
    );
    if (!res.ok) {
