    ...
    if live_pitches is None:
        ...
        try:
            ...
            if not target_game_pks:
                async with httpx.AsyncClient() as client:
                    # Fetch all schedule pages in parallel (was sequential, ~1s).
                    ...
                    sched_results = await asyncio.gather(
                        *[_fetch_sched(d) for d in check_dates]
                    )
                    sched_game_pks = []
                    ...
                    # Check boxscores in parallel to find games where this pitcher
                    # ACTUALLY PITCHED (not just on the roster).
